...
// Mock knowledge map data for CSCA subjects
...
export async function POST(request: Request) {
  try {
    ...
    const { subjects = ['基础汉语', '数学'] } = body;

    // Build combined knowledge map from subjects
    const knowledgeMap: any[] = [];
    subjects.forEach((subject: string) => {
      const topics = MOCK_TOPICS[subject] || MOCK_TOPICS['数学'];
